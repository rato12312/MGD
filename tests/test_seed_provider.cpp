...
bool run_seed_provider_tests() {
    ...
    // 8. Shader pelo polígono/posição: só o que está no raio pensado aquece
    {
        ...
        ASSERT_MSG(shaders2.lookup(mgd::shader::ShaderKey{55, 881, 0, 2}) != nullptr, "near ready at LOD 2");
        ASSERT_MSG(shaders2.lookup(mgd::shader::ShaderKey{55, 881, 0, 0}) == nullptr, "no LOD 0 entry for near");
        ASSERT_MSG(shaders2.lookup(mgd::shader::ShaderKey{55, 882, 0, 0}) == nullptr, "far untouched");
        ASSERT_MSG(shaders2.lookup(mgd::shader::ShaderKey{55, 882, 0, 2}) == nullptr, "far untouched at LOD 2");
    }
