...
// GLSL Compute Shader para FSR 2.x RCAS
/*
#version 460 core
...
void main() {
    ...
    // Center luminance
    float center_lum = dot(texture(input_image, vec2(gl_GlobalInvocationID.xy) / vec2(push.output_width, push.output_height)).rgb, vec3(0.299, 0.587, 0.114));
    ...
    // Apply sharpening
    vec3 center = texture(input_image, vec2(gl_GlobalInvocationID.xy) / vec2(push.output_width, push.output_height)).rgb;
    vec3 avg_neighbor = (neighbor[0] + neighbor[1] + neighbor[2] + neighbor[3]) / 4.0;
    vec3 sharpened = center + sharpening * (center - avg_neighbor);
