...
struct CameraOffsets {
    ...
    uint32_t fov_offset = 0;
    uint32_t aspect_offset = 0;
    uint32_t near_plane_offset = 0;
    uint32_t far_plane_offset = 0;
    uint32_t view_mode_offset = 0;
    uint32_t camera_type_offset = 0;
    uint32_t ortho_scale_offset = 0;
    uint64_t scene_graph_ptr = 0;
    uint32_t node_count_offset = 0;
    uint64_t root_node_offset = 0;
    uint32_t visible_count_offset = 0;
    uint32_t frustum_planes_offset = 0;
    uint32_t culling_system_offset = 0;
    uint32_t max_visible_nodes = 0;
    uint32_t culling_flags_offset = 0;
    uint32_t lod_distances_offset = 0;
...
struct OdysseyOffsets {
    struct CameraOffsets camera;
    struct SceneGraphOffsets scene_graph;
    struct CullingOffsets culling;
    OdysseyVersion version = OdysseyVersion::UNKNOWN;
...
public:
    ...
    // Extract all offsets from RAM dump
    bool extractOffsets(OdysseyOffsets& out);

    // Export formats
    bool exportToHeader(const OdysseyOffsets& offsets, const std::string& output_path);
    bool exportToJson(const OdysseyOffsets& offsets, const std::string& output_path);

    // Direct RAM access
    const uint8_t* ram() const { return ram_.data(); }
    size_t ramSize() const { return ram_size_; }
...
private:
    ...
    static constexpr uint8_t SCENE_SIG[] = {0x53, 0x63, 0x65, 0x6E, 0x65, 0x47, 0x72, 0x61, 0x70, 0x68}; // "SceneGraph"
    ...
    // Validation helpers
    bool isValidCameraPtr(uint64_t ptr) const;
    bool isValidCameraSystem(uint64_t ptr) const;
    bool isValidSceneGraph(uint64_t ptr) const;
    bool isValidSceneGraphPtr(uint64_t ptr) const;
