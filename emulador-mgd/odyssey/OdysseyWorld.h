...
public:
    ...
    // Painter apresenta: despeja o framebuffer em PPM (P6) com upscale
    // nearest (rascunho barato vira imagem cheia: Mali entrega pouco,
    // painter entrega grande).
    bool present(const char* path, int scale = 2) {
        if (scale < 1) scale = 1;
