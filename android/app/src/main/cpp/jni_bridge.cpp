...
static MarioOdysseyRunner* getRunner(jlong ptr) {
    return reinterpret_cast<MarioOdysseyRunner*>(ptr);
}
    ...
    if (window) {
        ...
        if (gpu.getVulkanContext()) {
            gpu.getVulkanContext()->initFromExisting(
                ...
                VK_NULL_HANDLE, VK_NULL_HANDLE // Will be set up properly by VulkanContext
            );
            // The VulkanContext needs the ANativeWindow to create surface
            // This is handled in VulkanContext::init()
        }
...
// PerformanceStats JNI helper
static jclass performanceStatsClass = nullptr;
static jmethodID performanceStatsConstructor = nullptr;

static void initPerformanceStatsClass(JNIEnv* env) {
    if (!performanceStatsClass) {
        jclass clazz = env->FindClass("com/mgd/odyssey/PerformanceStats");
        performanceStatsClass = (jclass)env->NewGlobalRef(clazz);
        performanceStatsConstructor = env->GetMethodID(performanceStatsClass, "<init>", "()V");
    }
}
...
Java_com_mgd_odyssey_MainActivity_nativeGetPerformanceStats(JNIEnv* env, jobject /* this */, jlong ptr) {
    ...
    initPerformanceStatsClass(env);
    
    const auto& stats = runner->getPerformanceStats();
    jobject obj = env->NewObject(performanceStatsClass, performanceStatsConstructor);
    
    auto setField = [&](const char* name, const char* sig, auto value) {
        jfieldID fid = env->GetFieldID(performanceStatsClass, name, sig);
        if (fid) env->SetField(obj, fid, value);
    };
